...
import '../../../data/owner/entity/subscriptions_entity.dart';
...
class _CompanySubscriptionState extends State<CompanySubscription> {
  ...
  void initState() {
    ...
    context.read<SubscriptionCubit>().emitGetSubscription(
        getSubscriptionParams: GetSubscriptionParams(categoryId: widget.categoryId));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Company Subscription', style: TextStyle(color: AppColors.semiDarkGolden, fontFamily: 'Bahnschrift', fontSize: 20.sp)),
        centerTitle: true,
        leading: IconButton(
          onPressed: () {
        ...
        print("Successfully loaded with selections: $selected");
              ...
              builder: (context, state) {
                return state.maybeWhen(
                  success: (subscriptions, selected) => buildSubscriptionList(subscriptions, selected),
  ...
  onTap: () {
    ...
    if (currentState is SuccessSubscription) {
      final selectedEntries = currentState.selectedSubscriptions.entries.where((entry) => entry.value);
      if (selectedEntries.isNotEmpty) {
        final subscriptionId = selectedEntries.first.key; 
        print(subscriptionId);
        context.read<SubscripeCubit>().emitSubscripe(subscripeParams: SubscripeParams(subscriptionId: subscriptionId));
  ...
  child: Container(
    ...
    decoration: BoxDecoration(
      gradient: LinearGradient(
        colors: [AppColors.mediumGolden1, AppColors.mediumGolden2, AppColors.mediumGolden1],
  ...
  onTap: () {
    ...
    if (currentState is SuccessSubscription) {
      final selectedEntries = currentState.selectedSubscriptions.entries.where((entry) => entry.value);
      if (selectedEntries.isNotEmpty) {
        final subscriptionId = selectedEntries.first.key; // Get the first selected subscription's ID
        context.read<SubscripeCubit>().emitSubscripe(subscripeParams: SubscripeParams(subscriptionId: subscriptionId));
        Fluttertoast.showToast(
  ...
  Widget buildSubscriptionList(BaseSubscriptionsEntity subscriptions, Map<int, bool> selected) {
    return ListView.separated(
      itemBuilder: (context, index) => SubScripChoiceItem(context, subscriptions.subscriptions[index], selected),
      separatorBuilder: (context, index) => SpaceItem(),
      itemCount: subscriptions.subscriptions.length,
    );
  }
Widget SubScripChoiceItem(BuildContext context, SubscriptionEntity subscription, Map<int, bool> selected){
  ...
  return Row(
    ...
    children: [
      Text(subscription.title, style: TextStyle(fontFamily: 'bahnschrift', fontSize: 16.sp)),
      Container(
        height: 30.h,
        width: 100.w,
        decoration: BoxDecoration(
      ...
      Checkbox(
        ...
        onChanged: (bool? value) {
          context.read<SubscriptionCubit>().toggleSubscription(subscription.id);
        },
      ),
    ],
  );
